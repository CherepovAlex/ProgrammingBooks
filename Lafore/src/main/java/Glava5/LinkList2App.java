...
public  class LinkList2App {
    public static void main(String[] args) {
        LinkList4 theList = new LinkList4();    // Создание нового списка
        ...
        Link4 f = theList.find(44);         // Поиск элемента
        if (f != null)
            System.out.println("Found link with key " + f.iData);
        else
            System.out.println("Can't find link");

        Link4 d = theList.delete(66);       // Удаление элемента
        if (d != null)
...
class Link4 {
    ...
    public double dDate;    // Данные
    public Link4 next;      // Следующий элемент в списке
//-------------------------------------------------------
    public Link4(int id, double dd) {   // Конструктор
        iData = id;
        dDate = dd;
    }
//-------------------------------------------------------
...
class LinkList4 {
    private Link4 first;        // Ссылка на первый элемент
//-------------------------------------------------------
    public LinkList4() {        // Конструктор
        first = null;           // Список пока не содержит элементов
    }
//-------------------------------------------------------
    public void insertFirst(int id, double dd) {
        Link4 newLink = new Link4(id, dd);  // Создание нового элемента
        newLink.next = first;               // newLink--> старое значение first
        first = newLink;                    // first--> newLink
    }
//-------------------------------------------------------
    public Link4 find(int key) {    // Поиск элемента с заданным ключом (предполагается, что список не пуст)
        Link4 current = first;       // Начаная с 'first'
        while (current.iData != key) {   // Пока совпадение не найдено
            ...
            else                        // Если ещё остались элементы
                current = current.next; // Перейти к следующему элементу
        }
        return current;                 // Совпадение обнаружено
    }
//-------------------------------------------------------
    public Link4 delete(int key) {      // Удаление элемента с заданным ключом (предполагается, что список не пуст)
        Link4 current = first;          // Поиск элемента
        Link4 previous = first;
