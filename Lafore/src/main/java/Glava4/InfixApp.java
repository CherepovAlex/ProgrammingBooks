package Glava4;
// Преобразование инфкисных арифметических выражений в постфиксные
...
class StackXI {
    ...
    //-------------------------------------------------
    public StackXI(int s) {     // Конструктор
        maxSize = s;
...
class InToPost {    // Преобразование инфиксной записи в постфиксную
    ...
    public void gotOper(char opThis, int prec1) {   // Чтение оператора из входной строки
        while (!theStack.isEmpty()) {
            ...
            if (opTop == '(') {      // Если это '('
                ...
                if (opTop == '+' || opTop == '-') // Определение оператора
                    prec2 = 1;
                else                   // Оператор
                    prec2 = 2;
    ...
    public void gotParen(char ch) {
        while (!theStack.isEmpty()) {   // Прочитана закрывающая скобка
            ...
            else                // Если извлечён оператор
                output = output + chx;  // Вывести в постфиксную форму
        }
    }
//-------------------------------------------------
    // Конец класс InToPost
}
