...
public class Example1 {
    ...
    public static void main(String[] args) {
        ...
        // Если на нужно просто асинхронно выполнить задачу и не нужно ничего возвращать
        // В виде лямбды передаём объект Runnable
        CompletableFuture<Void> future1 = CompletableFuture.runAsync(() -> {
            // Задержка имитирует длительно выполняющуюся операцию
            try {
                TimeUnit.SECONDS.sleep(1);
            } catch (InterruptedException e) {
                throw new IllegalStateException(e);
            }
            System.out.println("Run is separete thread");
        });
        // ждём окончания работы задачи
//        future1.get();
