...
Когда мы используем двойные кавычки для создания строки, то в пуле строк сначала ищется строка с таким же значение и если она найдены, то просто возвращается ссылка на нее, иначе в пуле создаётся новая строка, а затем возвращается ссылка на новую строку.
