...
[Раздел 11.16. Получение n-го символа в строке](#раздел-1116-получение-n-го-символа-в-строке)

[Раздел 11.17. Подсчет количества вхождений подстроки / символа в строку](#раздел-1117-подсчет-количества-вхождений-подстроки--символа-в-строку)
...
Для получения длины объекта `String` необходимо вызвать метод `length()`. Длина равна количеству кодовых единиц (символов) UTF-16 в строке.

```java
String str = "Hello World!";
System.out.println(str.length());   // Выведет число 13
```

Вы можете запустить эту программу онлайн на сайте ideone.com, перейдя по ссылке (https://ideone.com/0RWKcA).

Символ `char` в строке - это значение в формате UTF-16. Кодовые точки Юникода, значения которых >= 0x1000 (например, большинство эмодзи), используют две позиции символов. Для подсчета количества кодовых точек Unicode в строке, независимо от того, помещается ли каждая кодовая точка в значение символа UTF-16, можно использовать метод `codePointCount`:

```java
int length = str.codePointCount(0, str.length());
```

Начиная с Java 8, можно также использовать поток кодовых точек:
...
## Раздел 11.16. Получение n-го символа в строке
...
System.out.println(str.charAt(str.length - 1));   // Последний символ "g"
```

Чтобы получить символ в строке по индексу `n`, просто для строки `String` вызовите метод `charAt(n)`, где `n` - индекс символа, который необходимо получить.

_Примечание:_ индекс `n` начинается с 0, поэтому первый элемент расположен по индексу `n = 0`.

[к оглавлению Глава 11](#глава-11-строки)

## Раздел 11.17. Подсчет количества вхождений подстроки / символа в строку

Метод `countMatches` из класса `org.apache.commons.lang3.StringUtils` обычно используется для подсчета вхождений подстроки или символа в строку:
...
//подсчет вхождений символа char
char charTarget = ',';
inr charOccurrences = StringUtils.countMatches(text, charTarget);   // 3
```

Также для выполнения аналогичных действий со стандартными Java API можно использовать регулярные выражения:
...
String text = "One fish, two fish, red fish, blue fish";
System.out.println(countStringInString("fish", text));  // печатает 4
System.out.println(countStringInString(",", text));  // печатает 3
