...
Базовые операторы сдвига `>>` и `<<` являются _знаковыми_ операторами. Они сохраняют знак значения.
...
Чтобы присвоить какой-то переменной степень числа 2 (`2^n`), при работе с целыми числами можно использовать _операцию 
битового сдвига_, которая позволяет явно задать степень n.
...
int twoExp4 = 1<<6; // 2^6
...
int twoExp4 = 1<<31; // 2^31
```

Это особенно полезно для определения констант, у которых должно быть очевидно, что используется степень числа 2, а не в
шестнадцатеричной или десятичной системах счисления.

```java
Int twoExp4 = 0x10; // шестнадцатеричное значение
Int twoExp5 = 0x20; // шестнадцатеричное значение
int twoExp6 = 64;   // десятеричное значение
...
int twoExp31 = -2147483648;   // это степень числа 2?
```

Простой метод вычисления возведения числа int в степень, равную 2, будет выглядеть следующим образом:
...
## Раздел 22.6. Упаковка/распаковка битовых групп 

Обычно при работе с памятью несколько групп битовых значений сжимают в одно примитивное значение. Это может быть 
полезно для упаковки различной информации в одну переменную.

Например, в одну переменную типа `int` можно упаковать 3 байта - например, _код цвета в палитре RGB_.

**Упаковка значений**

```java
// Последовательность байтов в качесте входного сигнала
byte[] b = {(byte) 0x65, (byte) 0xFF, (byte)0x31};

// Упаковано в число с прямым порядком размещения байтов: x == 0x65FF31
int x = (b[0] & 0xFF) << 16 // Красный цвет
    |   (b[1] & 0xFF) << 8  // Зеленый
    |   (b[2] & 0xFF) << 0;  // Синий цвет

// Упаковано в число с обратным порядком размещения байтов: y == 0x31FF65
int x = (b[0] & 0xFF) << 0 
    |   (b[1] & 0xFF) << 8  
    |   (b[2] & 0xFF) << 16;
```
...
```java
// Значение int32 в качестве входного сигнала
int x = 0x31FF65;

// Распаковка в последовательность с порядком размещения байтов: {0x65, 0xFF, 0x31}
byte[] c = { 
        (byte)(x >> 16),
        (byte)(x >> 8),
        (byte)(x & 0xFF)
...
// Распаковка в последовательность с обратныс порядком размещения байтов: {0x31, 0xFF, 0x65}
byte[] c = { 
        (byte)(x & 0xFF),
        (byte)(x >> 8),
        (byte)(x >> 16)
...
[Раздел 22.6. Упаковка/распаковка битовых групп ](#раздел-226-упаковкараспаковка-битовых-групп-)
