...
Вы можете запустить этй программу онлайн на сайте `ideone.com`.<br>
И снова метод `main` внутри класса `Test` является точкой входа в нашу программу. Без метода `main` мы не можем указать виртуальной машине Java (JVM), с какого места начинать выполнение программы. <br>
