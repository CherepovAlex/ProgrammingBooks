...
Класс BigInteger имеет конструктор, предназначенный для генерации случайных больших чисел, на основе объекта 
`java.util.Random` и числа **int**, определяющего количество разрядов в результрирующем большом числе. Использовать его
довольно просто - при вызове конструктора `BigInteger(int, Random)` в виде:

```java
BigInteger randomBigInt = new BigInteger(bitCount, sourceOfRandomness);
```

вы получите число типа `BigInteger`, значение которого находится в диапазоне от `0` (включено в диапазон) до `2^bitCount`
(не включено а диапазон).

Это также означает, что имея достаточный запас времени на основе, например, конструктора 
`new BigInteger(2147483647, sourceOfRandomness)` можно получить все положительные значения типа `BigIntegers`.

Что использовать в качестве генератора случайности для параметра `sourceOfRandomness` - решать вам. Например, в 
большинстве случаев достаточно применить `new Random()`:

```java
new BigInteger(32, new Random());
```

Если вы готовы отказаться от быстродействи в угоду более высокому качеству случайных чисел, то можно использовать 
**new**. С документацией по использованию `SecureRandom()` можно ознакомиться по адресу 
"https://docs.oracle.com/javase/8/docs/api/java/security/SecureRandom.html"rel="nofollow%20noreferrer">.

Воспользоваться `SecureRandom()` можно, например, так:
...
Кроме того, с помощью анонимного класса вы можете реализовать свой собственный алгоритм! Обратите внимание, что 
использование **собственного алгоритма генерирования случайных чисел (RNG) приведёт к получению случайных чисел низкого
качества**, поэтому всегда используйте проверенные алгорится, если не хотите, чтобы получаемые псевдослучайные 
последовательности были слишком легко предсказуемы:
...
new BigInteger(32, new Random(){
    ...
    protected int next(int bits) {
        seed = ((22695477 * seed) + 1) & 2147483647;    // Значения, беззастенчиво
        // украденные из статьи, расположенной по адресу 
