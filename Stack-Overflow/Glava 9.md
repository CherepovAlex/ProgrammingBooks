...
+ 1. Каждый шестнадцатеричный литерал с плавающей запятой начинается с нуля (0), после которого следует символ `x` или `X`.
+ 2. В состав цифр числа (но не в экспоненту!) входят также шестнадцатеричные цифры от `a` до `f` и их эквиваленты в верхнем регистре.
+ 3. Экспонента является _обязательной_ и вводится буквой `p` (или `P`) вместе `e` или `E`. Экспоненты представляет собой показатель степени, который является степенью числа 2, а не степенью числа 10.
