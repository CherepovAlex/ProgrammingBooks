...
Java выполняет неявное приведене автоматически. Но если вам потребуется преобразование на сужение типов (и вы полность уверены, что потери данных при этом не произойдёт), то можно заставить Java выполнить преобразование с помощью языковой конструкции, известной как приведение.
...
int b = (int) d; // корректное преобразование в int, но результа получается
