...
Если мы не используем синтаксис команды `java -jar`, то команда `java` ищеn класс для загрузки путем поиска в параметре Classpath (см. раздел "Classpath). Приведённая выше команда полаагет на то, что по умолчанию путь к классу включает текущий каталог. Мы можем более явно указать используемый путь к классу с помощью опции `-cp`.
```bash
java -cp . HelloWorld
```

Это говорит о том, что текущи каталог (на который ссылается ".") является единственной записью в параметре Classpath.
