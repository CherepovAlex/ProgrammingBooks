...
[Раздел 8.6. Запуск исполняемого JAR-файла](#раздел-86-запуск-исполняемого-jar-файла)

[Раздел 8.7. Запуск Java-приложений с помощью класса, содержащего метод "main"](#раздел-87-запуск-java-приложений-с-помощью-класса-содержащего-метод-main)
...
Прежде всего, проблема обработки пробелов в аргументах на самом деле НЕ является проблемой в Java. Скорее, это проблема, которая должна решаться командной оболочкой, используемой при запуске Java-программы.

В качестве примера предположим, что у нас есть следующая простая программа, которая определяет и выводит размер файла:
...
Теперь предположим, что нам необходимо вывести размер файла, в имени которого присутствуют пробелы; например, `/home/steve/Test File.txt`. Если мы запустим команду следующим образом:

```bash
$ java PrintFileSizes /home/steve/Test File.txt
```

командная оболочка не будет знать, что `/home/steve/Test File.txt` - это путь, описываемый одной строкой. Вместо этого она передаст Java-приложению 2 разных аргумента. Приложение попытается найти соответствующие им размеры файлов и потерпит неудачу, поскольку файлов с такими путями (скорее всего) не существует.

**Решения с использованием командной оболочки POSIX**

К POSIX-оболочкам относят `sh`, а также производные от неё, такие как `bash` и `ksh`. Если вы используете одну из этих оболочек, то решить проблему можно, _заключив аргумент в кавычки_.

```bash
$ java PrintFileSizes "/home/steve/Test File.txt"
```

Двойные кавычки вокруг имени пути указывают оболочке, что строка должна быть передана как одинарный аргумент. При этом кавычки буду удалены. Существует и несколько других способов сделать это:

```bash
$ java PrintFileSizes '/home/steve/Test File.txt'
```

Одинарные (прямые) кавычки трактуются так же, как и двойные, за исключением того, что они также подавляют различные расширения внутри аргумента.

```bash
$ java PrintFileSizes /home/steve/Test\ File.txt
```

Обратный слэш экранирует следующий за ним пробел и не позволяет интерпретировать его как разделитель аргументов.

Более подробная документация, включая описание работы с другими специальными символами в аргументах, приведена в разделе по работе с кавычками документации по Bash.

**Решение для Windows**

Фундаментальная проблема для Windows заключается в том, что на уровне ОС аргументы передаются дочернему процессу в виде одной строка (`source`). Это означает, что конечная ответственность за разбор (или повторный разбор) командной строки ложится либо на саму программу, либо на её библиотеки. И здесь существует множество несоответствий.

Короче говоря, в случае с Java эту проблему можно решить следующим образом:
+ Вы можете поставить двойные кавычки вокруг аргумента в команде `java`, и это позволит передавать аргументы с пробелами.
+ По-видимому, команда `java` сама разбирает командную строку и делает это более или менее правильно.
+ Однако если попытаться совместить это с использованием команды `SET` и подстановкой переменных в файл `.bat`, то задача удаления двойных кавычек становится довольно сложной.
+ В командной оболочке `cmd.exe`, по-видимому, имеются другие механизмы экранирования, например, удвоение кавычек и использование символа `^`.

Более подробная информация приведена в документации по файлам с расширением `.bat`.

[к оглавлению Глава 8](#глава-8-команда-java---java-и-javaw)

## Раздел 8.6. Запуск исполняемого JAR-файла

Исполняемые JAR-файлы - это простейший способ собрать Java-код в один исполняемый файл.

Предположим, что у вас есть исполняемый JAR-файл с именем  **`<jar-path`**. Тогда вы можете запустить его на выполнение следующим образом:
```bash
java -jar <jar-path>
```

Если команда требует аргументов командной строки, добавьте из после **`<jar-path`**. Например:
```bash
java -jar <jar-path> arg1 arg2 arg3
```

Если в командной строке `java` необходимо указать дополнительные параметры для JVM, то они должны располагаться перед опцией `-jar`. Обратите внимание, что опция `-cp / -classpath` будет проигнорирована, если вы используете опцию `-jar`. Путь к классу приложения определяется манифестом JAR-файла.

[к оглавлению Глава 8](#глава-8-команда-java---java-и-javaw)

## Раздел 8.7. Запуск Java-приложений с помощью класса, содержащего метод "main"

Если приложение не было упаковано в виде исполняемого JAR-файла, то в командной строке вместе с `java` необходимо указать и имя класса, содержащего точку входа.

**Запуск класса HelloWorld**

Пример "HelloWorld" описан в разделе "Создание новой программы на Java" (Глава 1). Она состоит из одного класса, с именем `HelloWorld`, который удовлетворяет требованиям, предъявляемым к точке входа.

Если предположить, что скомпилированный файл "HelloWorld.class" находится в текущем каталоге, то запустить его можно следующим образом:
```bash
java HelloWorld
```
Отметим здесь следующие важные моменты:
+ При запуске мы должны указать именно имя класса, а не имя пути к файлу с расширением ".class" или файлу с расширением ".java".
+ Если класс объявлен в пакете (как и большинство классова Java), то имя класса, которое мы передаём в команде `java`, должно быть полным именем класса. Например, если `SomeClass` объявлен в пакете `com.example`, то полное имя класса будет `com.example.SomeClass`.

**Указание пути к классам**

Если мы не используем синтаксис команды `java -jar`, то команда `java` ищет класс для загрузки путем поиска в параметре Classpath (см. раздел "Classpath). Приведённая выше команда полагает на то, что по умолчанию путь к классу включает текущий каталог. Мы можем более явно указать используемый путь к классу с помощью опции `-cp`.
```bash
java -cp . HelloWorld
```
Это говорит о том, что текущий каталог (на который ссылается ".") является единственной записью в параметре Classpath.

Параметр `-cp` является опцией, которая обрабатывается командой `java`. Все опции, предназначенные для команды `java`, должны находиться перед именем класса. Все, что находится после имени класса, будет рассматриваться как аргумент командной строки для Java-приложения, который будет передан приложению в строке `String[]`, передаваемой в метод `main`.

(Если опция `-cp` не указана, то `java` будет использовать путь к классу, заданный переменной окружения `CLASSPATH`. Если эта переменная не задана или пуста, то `java` использует "." (текущий каталог) в качестве пути к классу по умолчанию).
