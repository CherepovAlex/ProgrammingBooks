...
public class StreamMiniTasksExtra {
    ...
    public static void main(String[] args) {
        ...
        List<Inventory> stock = List.of(
                ...
                new Inventory("P2", 10),
                new Inventory("P3", 5),
                new Inventory("P4", 320)
        );

        List<User> users = List.of(
                new User("U1", "anna@example.com", "Bishkek", 19),
                new User("U2", "anna@example.com", "Bishkek", 19),
        ...
        // 28. Оценка стоимости склада (stock value): sum(price * qty)
        // - вход: stock + productById
        // - приём: stream по складу -> умножаем BigDecimal -> reduce с identity BigDecimal.ZERO
        {
            BigDecimal totalStockValue = stock.stream()
                    // мапим позицию на складке в денежую стоимость: price(productId) * qty
                    .map(inv -> productById.get(inv.productId()).price.multiply(BigDecimal.valueOf(inv.qty)))
                    .reduce(BigDecimal.ZERO, BigDecimal::add);
            System.out.println("28) total stock value = " + totalStockValue);
        }

        // 29. Частоты слов (реальный кейс: подсчет частоты тегов/слов в логе/текстах).
        // - вход: список строк (слов)
        // - приём: toMap c merge-функцией или groupingBy + counting
        {
            List<String> words = List.of("apple", "banana", "apple", "cherry", "banana", "apple");
            // version 1: groupingBy + counting
            Map<String, Long> freq1 = words.stream().collect(groupingBy(Function.identity(), counting()));
            // version 2: toMap + merge (Long::sum)
        ...
        // - вход: logs
        // - приём: фильтр ERROR -> groupingBy(service, counting) -> сортировка по значению убыв. -> limit(2)
        {
            int N = 2;
            List<Map.Entry<String, Long>> topServiceByError = logs.stream()
                    ...
                    .limit(N)
                    .toList();
            System.out.println("30) top services by ERROR = " + topServiceByError);
        }
        ...
        // - приём: stream по entrySet() "базовой" мапы  + toMap(...)
        {
            ...
            Map<String, BigDecimal> discountPct = Map.of("P1", bd("10"), "P3", bd("5"));
            ...
            System.out.println("30) finalPrice = " + finalPrice);
        }

        // 32. Индекс по полю: Map<city, List<User> (с сортировкой внутри по email) + неизменямость
        ...
        // Реалистичнее было бы суммировать OrderItems * price, но для примера возьмём stock как «продажи».
        {
            ...
            List<Map.Entry<String, BigDecimal>> top3 = revenueByCategory.entrySet().stream()
