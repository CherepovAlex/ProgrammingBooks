package TwoPointers._11ContainerWithMostWater;
...
public class Solution {
    ...
    public int maxArea1(int[] height) {
        ...
        while (left < right) {  // Пока указатели не встретились
            // Определяем текущую высоту (минимум из двух линий)
            int currentHeight = Math.min(height[left], height[right]);
            int currentWidth = right - left; // Вычисляем текущую ширину
            int currentArea = currentHeight * currentWidth; // Вычисляем текущую площадь
            // Обновляем максимальную площадь если нужно
    ...
    public int maxArea2(int[] height) {
        ...
        while (left < right) {   // Пока указатели не встретились
            ...
            while (left < right && height[right] <= minHeight) {
                right--; // Пропускаем правые линии, которые не выше текущей минимальной
            }
        }
        // Возвращаем результат
        return maxArea;
