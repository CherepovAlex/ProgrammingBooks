package TwoPointers._160IntersectionOfTwoLinkedLists;
// Задача 160. Пересечение двух связанных списков
// Даны заголовки двух односвязных списков head A и head B.
...
public class Solution {
    ...
    public ListNode getIntersectionNode(ListNode headA, ListNode headB) {
        // Проверяем базовый случай - если один список пуст, пересечения нет
        if (headA == null || headB == null) return null;

        // Создаем два указателя для обхода списков
        ...
        while (pointerA != pointerB) {
            // Двигаем pointerA: если дошли до конца списка A,
            // переходим на начало списка B, иначе идем дальше по A
            pointerA = (pointerA == null) ? headB : pointerA.next;

            // Двигаем pointerB: если дошли до конца списка B,
            // переходим на начало списка A, иначе идем дальше по B
            pointerB = (pointerB == null) ? headA : pointerB.next;
        }
        // Возвращаем узел пересечения (или null, если списки не пересекаются)
        return pointerA;
    }

// Почему это работает:
//Компенсация разницы длин: Когда более короткий список заканчивается, его указатель переходит на начало более длинного списка, и наоборот.
//Гарантированная встреча: Если списки пересекаются, оба указателя пройдут одинаковое расстояние:
//Длина A + длина B до пересечения
//Оба пройдут через узел пересечения одновременно
//Обработка случая без пересечения: Если списки не пересекаются, оба указателя станут null одновременно после прохождения (m + n) узлов.
