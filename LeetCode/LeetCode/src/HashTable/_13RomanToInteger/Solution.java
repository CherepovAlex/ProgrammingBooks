package HashTable._13RomanToInteger;
// 13. Римские цифры обозначаются семью различными символами: I, V, X, L, C, D и M.
...
public class Solution {
    // time O(n), space O(n) - альтернативное решение с HashMap (менее оптимально по памяти)
    ...
    // time O(n), space O(1) - оптимальное решение
    public int romanToInt1(String s) {
        int result = 0; // Инициализируем переменную для хранения результата

        int prev = getValue(s.charAt(0)); // Получаем значение первого символа для начала сравнения

        for (int i = 1; i < s.length(); i++) {  // Проходим по строке начиная со второго символа
            int curr = getValue(s.charAt(i));   // Получаем значение текущего символа
            if (prev >= curr) {  // Если предыдущее значение больше или равно текущему - прибавляем его
                result += prev;
            } else {             // (римские цифры обычно идут в порядке убывания)
                result -= prev;  // Если предыдущее значение меньше текущего - вычитаем его
            }                    // Это случаи типа IV, IX, XL, XC, CD, CM
            prev = curr;         // Обновляем предыдущее значение для следующей итерации
        }
        result += prev;          // Добавляем значение последнего символа (он всегда прибавляется)
        return result;           // Возвращаем итоговый результат
    }
    // Вспомогательный метод для получения числового значения римской цифры
    private int getValue(char ch) {
        return switch (ch) {    // Используем switch expression для компактности и читаемости
            case 'I' -> 1;
            case 'V' -> 5;
            case 'X' -> 10;
            case 'L' -> 50;
            ...
            case 'D' -> 500;
            case 'M' -> 1000;
            default -> 0;       // На случай невалидного символа
        };
    }
