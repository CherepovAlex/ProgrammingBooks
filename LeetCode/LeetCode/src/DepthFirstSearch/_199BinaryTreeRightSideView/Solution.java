package DepthFirstSearch._199BinaryTreeRightSideView;
...
public class Solution {
    ...
    public List<Integer> rightSideView2(TreeNode root) {
        // Вызываем рекурсивный DFS обход, начиная с корня и уровня 0
        dfs(root, 1);
    ...
    private void dfs(TreeNode node, int depth) {
        ...
        //  Если текущая глубина равна размеру списка результат, это означает, что мы впервые достигли этого уровня
        // и текущий узел будет виден справа
        if (depth == result2.size()) {
            // Добавляеи значение узла в результат
            result2.add(node.val);
        }

        // Рекурсивно обходим сначала правого потомка (для правого вида)
        dfs(node.right, depth + 1);
        // Затем обходим левого потомка
