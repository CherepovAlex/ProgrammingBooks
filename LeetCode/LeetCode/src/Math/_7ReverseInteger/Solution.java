...
public class Solution {
    // version 1
    // O(Log 10 (X)) - обрабатываем каждую цифру числа, space O(1) - используем только константное количество переменных
    public int reverse(int x) {
        int reversed = 0;   // Инициализация переменной для хранения результата
        while (x != 0) {    // Пока в исходном числе еще есть цифры
            int pop = x % 10; // Получаем последнюю цифру (остаток от деления на 10)
            x = x / 10;       // Удаляем последнюю цифру из исходного числа
            ...
            // Перед умножением на 10 мы проверяем, не станет ли reversed меньше Integer.MIN_VALUE / 10
            ...
            // Сдвигаем существующие цифры влево (умножение на 10) и добавляем новую цифру
            reversed = reversed * 10 + pop;
        }
        return reversed;    // Возвращаем результат
    }
